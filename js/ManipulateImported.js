...
function SpawnHotspots(){
    ...
    scene.meshes.forEach(elem => {
        ...
        else if(elem.name == "Video_Screens"){
            console.log(elem)
            BABYLON.Tags.EnableFor(elem)
            BABYLON.Tags.AddTagsTo(elem, "screen_link");
            elem.isPickable = true;
            elem.actionManager = new BABYLON.ActionManager(scene);
	
            //ON MOUSE ENTER
            elem.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnPointerOverTrigger, function(ev){	
                //elem.material.emissiveColor = BABYLON.Color3.Blue();
